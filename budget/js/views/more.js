...
import { getSettings, saveSettings, exportAllData, importAllData, resetAllData, getExpenses, getCategories } from '../db.js';
...
export function render() {
  ...
  document.getElementById('view').innerHTML = `
    ...
    <div class="card">
      <div class="chip-row">
        ${CURRENCIES.map((c) => `<button class="chip ${settings.currency === c ? 'active' : ''}" data-currency="${c}">${c}</button>`).join('')}
      </div>
    </div>
    ...
    <div class="card stack">
      ...
      <button class="btn btn-ghost" id="export-csv">Ausgaben als CSV exportieren</button>
      <button class="btn btn-danger" id="reset-all">Alle Daten löschen</button>
    </div>

    <div class="section-title">Über die App</div>
    <div class="card">
      <p class="faint">Budget · Version 1.0</p>
      <p class="faint" style="margin-top:6px">Alle Daten bleiben ausschließlich lokal auf diesem Gerät gespeichert. Es findet keine Cloud-Synchronisation statt.</p>
    </div>
  `;
  ...
  document.querySelectorAll('[data-currency]').forEach((el) => {
    el.addEventListener('click', () => { saveSettings({ currency: el.dataset.currency }); render(); });
  });

  document.getElementById('export-json').addEventListener('click', () => {
    const data = exportAllData();
    download(`budget-backup-${todayKey()}.json`, JSON.stringify(data, null, 2));
  ...
  document.getElementById('export-csv').addEventListener('click', () => {
    ...
    const rows = [['Datum', 'Kategorie', 'Betrag', 'Händler', 'Notiz', 'Wiederkehrend', 'Steuerlich relevant']];
    for (const e of getExpenses().sort((a, b) => a.date.localeCompare(b.date))) {
      const cat = categories.find((c) => c.id === e.categoryId);
      rows.push([e.date, cat?.name || e.categoryId, e.amount, e.merchant, e.note, e.recurring ? 'ja' : 'nein', e.taxRelevant ? 'ja' : 'nein']);
    }
    const csv = rows.map((r) => r.map(csvEscape).join(';')).join('\n');
    download(`budget-export-${todayKey()}.csv`, csv, 'text/csv');
    toast('CSV exportiert');
  });

  document.getElementById('reset-all').addEventListener('click', async () => {
    const ok = await confirmDialog('Wirklich alle Daten löschen?', 'Alle Ausgaben und Kategorien werden unwiderruflich gelöscht.', 'Alles löschen', true);
    if (!ok) return;
