...
function draw() {
  ...
  const assignedNotes = getNotesForApp('household');
...
function openAssignedNoteModal(note) {
  ...
  handle.sheet.querySelectorAll('[data-an-item]').forEach((cb) => {
    cb.addEventListener('change', () => {
      updateAssignedNoteContent(note.id, 'household', { itemId: cb.dataset.anItem, itemDone: cb.checked });
    });
  });
  handle.sheet.querySelector('#an-save').addEventListener('click', () => {
    ...
    updateAssignedNoteContent(note.id, 'household', patch);
    toast('Gespeichert');
    handle.close();
    draw();
  });
  handle.sheet.querySelector('#an-unassign').addEventListener('click', async () => {
    ...
    if (!ok) return;
    unassignNote(note.id, 'household');
