...
function draw() {
  ...
  const links = getLinks().filter((l) => idSet.has(l.personIdA) && idSet.has(l.personIdB));

  if (!people.length) {
    view.innerHTML = `<div class="empty"><h3>Noch keine Job-Kontakte</h3><p class="faint">Lege Job-Kontakte an und verknüpfe sie (z.B. drüben in Social), um das Netzwerk zu sehen.</p></div>`;
    return;
  }
 ...
 * keine externe Bibliothek) - identisch zu social/js/views/network.js, hier
 * nur auf Personen mit jobProfile eingeschraenkt.
 */
function layout(people, links) {
  ...
  const k = Math.sqrt(area / Math.max(1, n)) * 0.8;
