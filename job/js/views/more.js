...
export function render() {
  ...
  document.getElementById('view').innerHTML = `
    ...
    <div class="card stack">
      <button class="btn btn-ghost" id="export-json">Termine-Backup exportieren (JSON)</button>
      ...
      <button class="btn btn-danger" id="reset-all">Alle Job-Daten löschen</button>
    </div>
    <p class="faint" style="margin:-6px 0 20px">Betrifft nur Termine und Job-Profile. Kontakte selbst (Name, Social-Profil, Beziehungs-Log) gehören der Social-App - deren Backup dort exportieren.</p>

    <div class="section-title">Über die App</div>
    <div class="card">
      <p class="faint">Job · Version 1.0</p>
  ...
  document.getElementById('export-json').addEventListener('click', () => {
    ...
    download(`job-backup-${todayKey()}.json`, JSON.stringify(data, null, 2));
  ...
  document.getElementById('import-json').addEventListener('change', async (e) => {
    ...
    if (!file) return;
    const ok = await confirmDialog('Backup importieren?', 'Vorhandene Termine/Einstellungen werden überschrieben. Fahre nur fort, wenn du dem Ursprung dieser Datei vertraust.', 'Importieren', false);
    if (!ok) return;
    try {
  ...
  document.getElementById('reset-all').addEventListener('click', async () => {
    const ok = await confirmDialog('Wirklich alle Job-Daten löschen?', 'Termine werden geloescht, Job-Profile von allen Kontakten entfernt. Social-Kontakte/Beziehungs-Log bleiben erhalten.', 'Alles löschen', true);
    if (!ok) return;
    ...
    resetAllData();
    toast('Job-Daten wurden gelöscht');
    location.hash = '#/';
