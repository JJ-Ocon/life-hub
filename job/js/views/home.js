...
function drawAssignedNotes() {
  ...
  if (!el) return;
  const assignedNotes = getNotesForApp('job');
...
function openAssignedNoteModal(note) {
  ...
  handle.sheet.querySelectorAll('[data-an-item]').forEach((cb) => {
    cb.addEventListener('change', () => {
      updateAssignedNoteContent(note.id, 'job', { itemId: cb.dataset.anItem, itemDone: cb.checked });
    });
  });
  handle.sheet.querySelector('#an-save').addEventListener('click', () => {
    ...
    updateAssignedNoteContent(note.id, 'job', patch);
    toast('Gespeichert');
    handle.close();
    drawAssignedNotes();
  });
