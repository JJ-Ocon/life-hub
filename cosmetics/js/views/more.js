...
export function render() {
  ...
  document.getElementById('view').innerHTML = `
    ...
    <div class="card">
      <p class="faint">Kosmetik · Version 1.0</p>
  ...
  document.getElementById('export-json').addEventListener('click', () => {
    ...
    download(`kosmetik-backup-${todayKey()}.json`, JSON.stringify(data, null, 2));
  ...
  document.getElementById('reset-all').addEventListener('click', async () => {
    const ok = await confirmDialog('Wirklich alle Daten löschen?', 'Alle Pflegeprodukte werden unwiderruflich gelöscht.', 'Alles löschen', true);
    if (!ok) return;
