...
import { getSettings, saveSettings, lockVault, changePassphrase, exportEncryptedBackup, importEncryptedBackup, resetVault } from '../db.js';
...
export function render() {
  ...
  document.getElementById('view').innerHTML = `
    ...
    <div class="card">
      <p class="faint">Digitaler Safe · Version 1.0</p>
      <p class="faint" style="margin-top:6px">Dokumente werden clientseitig verschlüsselt (AES-GCM) und bleiben ausschließlich auf diesem Gerät. Ohne Passphrase gibt es keine Wiederherstellung.</p>
    </div>
  `;
  ...
  document.getElementById('export-json').addEventListener('click', () => {
    ...
    download(`digitaler-safe-backup-${todayKey()}.json`, JSON.stringify(data, null, 2));
  ...
  document.getElementById('reset-all').addEventListener('click', async () => {
    const ok = await confirmDialog('Vault wirklich vollständig löschen?', 'Alle Dokumente werden unwiderruflich gelöscht. Dies kann nicht rückgängig gemacht werden.', 'Alles löschen', true);
    if (!ok) return;
    const typed = await promptDeleteConfirm();
...
function promptDeleteConfirm() {
  return new Promise((resolve) => {
    const handle = openModal(`
      <h3 class="modal-title">Zur Bestätigung "LÖSCHEN" eingeben</h3>
      <input class="input" id="del-confirm" placeholder="LÖSCHEN" style="margin-bottom:16px">
      <div class="row" style="gap:10px">
        <button class="btn btn-ghost" data-act="cancel">Abbrechen</button>
        <button class="btn btn-danger" data-act="ok">Löschen</button>
      </div>
    `, { center: true, onClose: () => resolve(null) });
    const input = handle.sheet.querySelector('#del-confirm');
    input.focus();
    handle.sheet.querySelector('[data-act="cancel"]').addEventListener('click', () => { resolve(null); handle.close(); });
    handle.sheet.querySelector('[data-act="ok"]').addEventListener('click', () => { resolve(input.value.trim()); handle.close(); });
  });
}
